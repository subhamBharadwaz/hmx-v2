...
const getProducts = async () => {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_API_URL}/api/v1/products?gender=Men`
  )
...
export default async function MenProductsPage() {
  ...
  await queryClient.prefetchQuery(["products-Men"], getProducts)
    ...
    <section>
       ...
       <div className="container mb-16"  >
        ...
        <div className="mb-16 flex h-56 w-full flex-col items-center justify-center gap-y-10 bg-zinc-800 dark:bg-slate-100 lg:h-72">
          <p className="font-serif text-2xl tracking-tighter text-slate-200 dark:text-slate-600 lg:text-4xl">&quot;A Swiss Army knife for your closet.&quot;</p>
          <div className="flex gap-5">
          <p className="font-serif text-xl tracking-tighter text-slate-200 dark:text-slate-600  lg:text-2xl">VOGUE</p>
          <p className="text-xl font-bold tracking-tighter text-zinc-400  lg:text-2xl">WWD</p>
          </div>
